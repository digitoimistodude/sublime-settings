...
**Setting up:** Copy file [`FileHeader.sublime-settings`](https://raw.githubusercontent.com/digitoimistodude/sublime-settings/master/settings/FileHeader.sublime-settings) (comes with all the settings with this repo) to your `Packages/User/`, edit your mac username in place to that file, then copy all the templates in `fileheaders` folder to your `Packages/FileHeader/template/` directory, backup and override existing.
